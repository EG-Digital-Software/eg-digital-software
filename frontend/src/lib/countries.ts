// AUTO-GENERATED from flag-icons' ISO country list + E.164 dial codes.
// Regenerate rather than hand-editing entries.

export interface Country {
  /** ISO-3166 alpha-2, upper case — the value persisted in the database. */
  code: string;
  name: string;
  /** E.164 country calling code, with the leading plus. */
  dial: string;
}

/** Every ISO country, alphabetical by name. */
export const COUNTRIES: Country[] = [
  ...
  { code: 'FM', name: "Federated States of Micronesia", dial: '+691' },
  ...
  { code: 'SH', name: "Saint Helena, Ascension and Tristan da Cunha", dial: '+290' },
  ...
  { code: 'GS', name: "South Georgia and the South Sandwich Islands", dial: '+500' },
  { code: 'KR', name: "South Korea", dial: '+82' },
  { code: 'SS', name: "South Sudan", dial: '+211' },
  { code: 'ES', name: "Spain", dial: '+34' },
  { code: 'LK', name: "Sri Lanka", dial: '+94' },
  { code: 'PS', name: "State of Palestine", dial: '+970' },
  ...
  { code: 'UM', name: "United States Minor Outlying Islands", dial: '+1' },
...
/** Default market for new records. */
...
export function dialCodeOf(code?: string | null): string {
  return countryByCode(code)?.dial ?? countryByCode(DEFAULT_COUNTRY)!.dial;
}
...
/** CSS class for the flag sprite (flag-icons), e.g. "fi fi-au". */
export function flagClass(code?: string | null): string {
  return `fi fi-${(code || DEFAULT_COUNTRY).toLowerCase()}`;
}

/** Reverse lookup for addresses, which persist the country's display name. */
const BY_NAME = new Map(COUNTRIES.map((c) => [c.name.toLowerCase(), c]));

export function countryCodeByName(name?: string | null): string {
  if (!name) return DEFAULT_COUNTRY;
  return BY_NAME.get(name.trim().toLowerCase())?.code ?? DEFAULT_COUNTRY;
}
